...
// Function to parse hotkey string (e.g., "Ctrl+Alt+P")
bool ParseHotkey(const std::string& hotkeyStr, UINT& modifiers, UINT& vkCode) {
    ...
    if (lastPlus != std::string::npos && lastPlus < str.length() - 1) {
        std::string key = str.substr(lastPlus + 1);
        key = Trim(key);
        ...
        case WM_USER + 1: // Tray icon message
            if (lParam == WM_RBUTTONUP) {
                ...
                if (cmd == 1) {
                    ...
                    std::string editorPath = std::string(path) + "\\config-editor.html";
                    ...
                    if (file.is_open()) {
                        ...
                        for (const auto& pair : g_appConfigs) {
                            ...
                            hotkey += (char)pair.second.vkCode;
